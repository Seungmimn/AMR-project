...
void setup()
{
  ...
  //nh.subscribe(cmd_vel_sub); //unactive -KGB
  nh.subscribe(sound_sub);
  nh.subscribe(motor_power_sub);
  nh.subscribe(reset_sub);
  nh.subscribe(sub_cmd_vel); // another code for R model +KGB
  nh.subscribe(sub_cmd_vel_lift);  // addtional ros node +KGB
...
/*******************************************************************************
* Callback function for cmd_vel msg // unactivate -KGB
*******************************************************************************/
...
/*******************************************************************************
* + func callback & motor speed set +KGB
*******************************************************************************/
void cmdVelCallback(const geometry_msgs::Twist& cmd_vel) {
  float linear_x = cmd_vel.linear.x;   // 전진/후진
  float angular_z = cmd_vel.angular.z; // 좌/우 회전

  // 전진/후진 명령 처리
  int forward_speed = map(linear_x * 255, -1.0, 1.0, -255, 255);
  if (linear_x < 0) {
    movingForward = true;
  ...
  // 좌/우 회전은 PWM 신호로 처리 (일정 시간 제어가 아닌 경우)
  int turning_speed = -map(angular_z * 255, -1.0, 1.0, -255, 255);
  if (angular_z != 0) {
    turning = true;
    turnStartTime = millis();  // 좌우 회전 시작 시간 기록
    setMotorSpeed(motorPin3, motorPin4, turning_speed);
  } else {
    turning = false;
  }
  // 전방 바퀴 제어 (전진/후진)
  setMotorSpeed(motorPin1, motorPin2, forward_speed);

}

void cmdVelLiftCallback(const geometry_msgs::Twist& cmd_vel_lift) {
  float linear_z = cmd_vel_lift.linear.z;   // 지게 위아래

  // lift up/down 명령 처리
  int lift_speed = map(linear_z * 255, -1.0, 1.0, -255, 255);
  if (linear_z < 0) {
    liftingUp = true;
    liftingDown = false;
    liftMoveStartTime = millis();  // 지게차 동작 시작 시간 기록
  } else if (linear_z > 0) {
    liftingUp = false;
    liftingDown = true;
    liftMoveStartTime = millis();  // 지게차 동작 시작 시간 기록
  ...
  // 지게차 조향
  setLiftSpeed(ENA, IN1, IN2, lift_speed);
}

void pose_operate() // forward/backward move for a period time +KGB
{
  // 일정 시간 동안만 전진/후진 동작
  if (movingForward || movingBackward) {
    if (millis() - moveStartTime < moveDuration) {
      setMotorSpeed(motorPin1, motorPin2, movingForward ? 255 : -255);
      
      if (movingForward) {
        beepBuzzer(); // 후진 시 부저 울림
      }
    } else {
      setMotorSpeed(motorPin1, motorPin2, 0);  // 모터 정지
      movingForward = false;
      movingBackward = false;
    }
  } else {
  ...
  if (turning) {
    if (millis() - turnStartTime < turnDuration) {
      ...
      setMotorSpeed(motorPin3, motorPin4, 0);  // 좌우 회전 모터 정지
      turning = false;
    }
  } else {
    setMotorSpeed(motorPin3, motorPin4, 0);  // 좌우 회전 모터 정지
...
void lift_operate() // lift up/down operate for a period time +KGB
{
  if (liftingUp || liftingDown) {
    if (millis() - liftMoveStartTime < liftMoveDuration) {
      setLiftSpeed(ENA, IN1, IN2, liftingUp ? 255 : -255);
      
      if (liftingUp || liftingDown) {
        beepBuzzer(); // 지게를 올릴 때 부저 울림
      }
    } else {
      setLiftSpeed(ENA, IN1, IN2, 0);  // 지게차 정지
      liftingUp = false;
      liftingDown = false;;
    }
  } else {
...
void setMotorSpeed(int pin1, int pin2, int speed) {
  if (speed > 0) {
    analogWrite(pin1, speed);  // 전진 (PWM 신호)
    analogWrite(pin2, 0);
  } else if (speed < 0) {
    analogWrite(pin1, 0);
    analogWrite(pin2, -speed);  // 후진 (PWM 신호)
  } else {
    analogWrite(pin1, 0);  // 정지
    analogWrite(pin2, 0);
  }
}
...
void setLiftSpeed(int pin1, int pin2, int pin3, int speed) {
  if (speed > 0) {
    analogWrite(pin1, speed);  // 상승 (PWM 신호)
    digitalWrite(pin2, LOW);
    digitalWrite(pin3, HIGH);
  } else if (speed < 0) {
    analogWrite(pin1, -speed);  // 하강 (PWM 신호)
...
/*******************************************************************************
* + func callback & motor speed set end +KGB
*******************************************************************************/
