...
RosNode::RosNode(int argc, char** argv)
{
    ...
    cmd_vel_lift_pub =
          nh.advertise<geometry_msgs::Twist>("cmd_vel_lift", 1);
    lds_sub =
...
void RosNode::run()
{
    ...
    while(ros::ok())
    {
        ...
        if(liftCmdFlag)
        {
            cmd_vel_lift_pub.publish(lift_cmd);
        }
        ros::spinOnce();
        loop_rate.sleep();
    }
...
//---HCU추가
  ...
  scanData[1] = msg->ranges[60];
  scanData[2] = msg->ranges[120];
  scanData[3] = msg->ranges[180];
  scanData[4] = msg->ranges[240];
  scanData[5] = msg->ranges[300];
    //  qDebug() << "scanData[0] : " << scanData[0];

  // 장애물이 0.25미터 이내에 있으면 로봇을 멈춤
  if ((scanData[0] >0 && scanData[0]< 0.55 && base_cmd.linear.x > 0) ||
     (scanData[1] > 0 && scanData[1] < 0.15 && base_cmd.linear.x > 0)||
     (scanData[2] > 0 && scanData[2] < 0.15 && base_cmd.linear.x < 0)||
     (scanData[3] > 0 && scanData[3] < 0.5 && base_cmd.linear.x < 0)||
     (scanData[4] > 0 && scanData[4] < 0.15 && base_cmd.linear.x < 0)||
     (scanData[5] > 0 && scanData[5] < 0.15 && base_cmd.linear.x > 0)){
      base_cmd.linear.x = 0.0;
      base_cmd.angular.z = 0.0;
      cmd_vel_pub.publish(base_cmd);
  }
...
void RosNode::set_cmd_vel_lift(geometry_msgs::Twist liftCmd)
{
    lift_cmd.linear.z = liftCmd.linear.z;
}
...
void RosNode::set_cmdLiftCmd(bool flag)
{
    liftCmdFlag = flag;
}
