...
void setup()
{
  ...
  nh.subscribe(sub_lift); //add additional lift motor
...
/*******************************************************************************
* Callback function for lift motor msg
*******************************************************************************/
void cmdVelliftCallback(const geometry_msgs::Twist& cmd_vel_lift) {
  // float linear_x = cmd_vel.linear.x;   // 전진/후진
  // float angular_y = cmd_vel.angular.y; // 좌/우 회전
  float linear_z = cmd_vel_lift.linear.z;   // 지게 위아래


  // 좌/우 회전은 PWM 신호로 처리 (일정 시간 제어가 아닌 경우)
  // int turning_speed = map(angular_y * 255, -1.0, 1.0, -255, 255);
  // setMotorSpeed(motorPin3, motorPin4, turning_speed > 0, turning_speed < 0);
...
*******************************************************************************/
void setLiftSpeed(int pin1, int pin2, int pin3, bool up, bool down) {
  if (up) {
    digitalWrite(pin2, LOW);
    digitalWrite(pin3, HIGH);  // 상승
    analogWrite(pin1, 255);    // 최대 속도로 상승
  } else if (down) {
    digitalWrite(pin2, HIGH);
    digitalWrite(pin3, LOW);   // 하강
    analogWrite(pin1, 255);    // 최대 속도로 하강
  } else {
...
/*******************************************************************************
* lift motor operation msg
*******************************************************************************/
void lift_operate(void)
{
  if (liftingUp || liftingDown) {
    if (millis() - liftMoveStartTime < liftMoveDuration) {
      setLiftSpeed(ENB, IN3, IN4, liftingUp, liftingDown);
    } else {
      setLiftSpeed(ENB, IN3, IN4, false, false);  // 지게차 정지
      liftingUp = false;
      liftingDown = false;
    }
  } else {
